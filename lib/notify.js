...
function ticketFlex(t, headline, accent, opts = {}) {
  ...
  // ปุ่มท้ายการ์ด: เปิดหน้าเว็บใบนี้ (สำหรับช่าง/แอดมิน) + ยืนยันปิดงาน (สำหรับผู้แจ้ง)
  const footerButtons = [];
  if (opts.openButton) footerButtons.push({
    type: 'button', style: 'primary', color: '#6366f1', height: 'sm',
    action: { type: 'uri', label: '🔧 เปิด / อัปเดตสถานะ', uri: `${PUBLIC_BASE}/?ticket=${encodeURIComponent(t.no)}` },
  });
  if (opts.confirmButton) footerButtons.push({
...
export async function notifyNew(t) {
  ...
  // ประหยัดโควตา: แจ้งซ่อมใหม่ -> push เข้ากลุ่มอย่างเดียว (ไม่ DM ผู้แจ้ง)
  const results = await Promise.all([
    sendEmail(`[แจ้งซ่อม] ${t.no} - ${t.equipmentName}`, text),
    pushLine(group, ticketFlex(t, '🔧 มีการแจ้งซ่อมใหม่', null, { openButton: true })),
  ]);
  console.log('[notifyNew]', t.no, JSON.stringify(results));
  return results;
}

export async function notifyStatus(t) {
  const text = ticketText(t, `📌 อัปเดตสถานะงานซ่อม → ${t.status}`);
  // อีเมลส่งทุกครั้งที่เปลี่ยนสถานะ (ไม่กินโควตา LINE)
  const tasks = [sendEmail(`[อัปเดต ${t.status}] ${t.no} - ${t.equipmentName}`, text)];
  // ประหยัดโควตา: push LINE หาผู้แจ้ง "เฉพาะตอนสำเร็จ" (พร้อมปุ่มยืนยันปิดงาน) เท่านั้น
  const user = lineUserOf(t);
  if (user && t.status === 'สำเร็จ') {
    tasks.push(pushLine(user, ticketFlex(t, '🔔 งานที่คุณแจ้งซ่อมเสร็จแล้ว', null, { confirmButton: true })));
  }
  const results = await Promise.all(tasks);
  console.log('[notifyStatus]', t.no, '→', t.status, 'reporterPush=' + (user && t.status === 'สำเร็จ' ? 'yes' : 'no'), JSON.stringify(results));
  return results;
}
...
export async function notifyConfirmed(t) {
  const group = await groupTarget();
  const r = await pushLine(group, ticketFlex(t, '✅ ผู้แจ้งยืนยันปิดงานแล้ว', '#16a34a', { openButton: true }));
