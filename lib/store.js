...
function jsonSave(d) { fs.writeFileSync(DB_FILE, JSON.stringify(d, null, 2)); }
...
export async function updateTicket(id, patch) {
  ...
  ['assignee', 'solution', 'detail', 'priority', 'problemType'].forEach((k) => { if (patch[k] !== undefined) t[k] = patch[k]; });
  if (statusChanged) {
  ...
  if (db.enabled) {
    await db.q(
      `update tickets set assignee=$2,solution=$3,detail=$4,priority=$5,problem_type=$6,status=$7,history=$8,updated_at=$9,closed_at=$10 where id=$1`,
      [t.id, t.assignee, t.solution, t.detail, t.priority, t.problemType, t.status, JSON.stringify(t.history), t.updatedAt, t.closedAt]
    );
